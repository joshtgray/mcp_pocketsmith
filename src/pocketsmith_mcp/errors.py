...
def validate_event_id(value: str, field_name: str) -> str:
    """Validate that an event ID is a plain integer string or composite series_id-timestamp string.

    Args:
        value: The event ID string to validate.
        field_name: Name of the field (for error messages).

    Returns:
        The validated event ID string, unchanged.

    Raises:
        ValidationError: If value is empty, whitespace-only, non-numeric,
            or has more than two parts.
    """
    ...
    if len(parts) > 2:
        raise ValidationError(
            f"{field_name} must be a plain integer or composite ID, got {value!r}",
            field=field_name,
        )
    for part in parts:
        if not part.isdigit():
...
_VALID_BEHAVIOURS = {"one", "forward", "all"}


def validate_behaviour(value: str, field_name: str) -> str:
    """Validate that a behaviour string is one of the accepted PocketSmith event values.

    Args:
        value: The behaviour value to validate.
        field_name: Name of the field (for error messages).

    Returns:
        The validated behaviour string, unchanged.

    Raises:
        ValidationError: If value is not one of 'one', 'forward', 'all'.
    """
    if value not in _VALID_BEHAVIOURS:
        raise ValidationError(
            f"{field_name} must be one of {sorted(_VALID_BEHAVIOURS)!r}, got {value!r}",
            field=field_name,
        )
    return value
